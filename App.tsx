...
export default function App() {
  ...
  useEffect(() => {
      ...
      const move = (e: MouseEvent | TouchEvent) => {
          ...
          if(isResizingHeight) {
             // For mobile vertical resizing
             // The map is order-1 (top), Sidebar is order-2 (bottom).
             // Resizer is between them.
             // clientY is the position of the touch/mouse.
             const clientY = 'touches' in e ? e.touches[0].clientY : (e as MouseEvent).clientY;
             const hPercent = (clientY / window.innerHeight) * 100;
             
             // Limit bounds: Map min 20%, Sidebar min 20%
             if(hPercent > 20 && hPercent < 80) {
                 // Map height is roughly this percent (minus header). 
                 // Actually, we are setting sidebarHeight. Sidebar is at bottom.
                 // So sidebar height is (100 - hPercent).
    ...
    <div className="flex flex-col md:flex-row h-screen w-screen overflow-hidden font-sans text-[#333]">
      ...
      >
        <div className="bg-[#2c3e50] text-white p-4 shrink-0">
          ...
          {/* Sidebar Filter Bar (Multi-Select) */}
